...
import type { EdgeTypes, NodeTypes, Node, Edge, FitViewOptions, InternalNode } from '../types';
...
import type { StoreSignals, NgFlowStore, FlowStoreActions } from './types';

export { FLOW_STORE_CONTEXT, injectStore } from './context';
export { FlowStore, getInitialStore, initialEdgeTypes, initialNodeTypes } from './initial-store';

export function createStore<NodeType extends Node = Node, EdgeType extends Edge = Edge>(
  signals: StoreSignals<NodeType, EdgeType>
): NgFlowStore<NodeType, EdgeType> {
  ...
  function fitView(options?: FitViewOptions<NodeType>) {
    ...
    /*
     * We need to re-run adoptUserNodes so the fit view request is picked up.
     * Unlike Svelte, Angular propagates model inputs asynchronously (during
     * change detection), so this is deferred one tick: a synchronous
     * `store.nodes = [...store.nodes]` here would read the stale model value
     * and clobber nodes the user just set before calling fitView.
     */
    setTimeout(() => {
      store.retriggerAdoptUserNodes();
    });
  ...
  const storeWithActions = Object.assign(store, {
    ...
    cancelConnection,
    reset
  } satisfies FlowStoreActions<NodeType, EdgeType>);
